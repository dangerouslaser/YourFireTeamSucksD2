angular.module('fireTeam.common')
	...
	MainCtrl.$inject = ['$scope','PlayerBaseModelFactory', 'GameActivityModelFactory', 'FireTeamModelFactory', 'PlayerOptionsService', 'ActivityModelFactory', '$timeout'];

	function MainCtrl($scope, playerBaseModelFactory, gameActivityModelFactory, fireTeamModelFactory, playerOptionsService, activityModelFactory, $timeout) {

		var m = $scope.m = {
			fireTeamActivityResults: null,
			playersArrays: [{
				displayName: '',
				isPlaceHolder: true
			}],
		...
		m.activityListProgress = 0;

		$scope.getPlayerInfo = getPlayerInfo;
		...
		$scope.$watch('m.playersArrays', function(newVal){
			...
			var indexToDelete = -1;
			angular.forEach(newVal, function(input, index){
			...
			if(placeholderCount > 1 && indexToDelete > -1){
				m.playersArrays.splice(indexToDelete, 1);
			}
		...
		function getFireTeamModel(memberType){
			...
			fireTeamModelFactory.getFireTeam(memberType, m.playersArrays).then(function(response){
		...
		function getActivityStats(){
			...
			angular.forEach(m.fireTeamMembers, function(player){
				instancesObject[player.membershipInfo.displayName] = [];
				angular.forEach(player.characters, function(character){
					var data = {membershipId: player.membershipId, characterId: character.characterBase.characterId, mode:gameMode, page: pageCount};
					playerOptionsService.getActivityHistoryData(data).then(function(response){
						character.activities = response.data.Response.data.activities;
						angular.forEach(character.activities, function(event){
							var instance = event.activityDetails.instanceId;	
							instancesObject[player.membershipInfo.displayName].push(instance);
						});
					});
				});
			});
		...
		function getFireTeamInstanceData(instanceIdArray){
			...
			//TODO set a timeout/interval to check progress here;
			startPollingForProgress();
		}

		function startPollingForProgress(){
			$timeout(function() {
				m.activityListProgress = activityModelFactory.getProgress();
				if(m.isLoadingData){
					startPollingForProgress();
				}
			}, 200);
		...
		function getPlayerInfo(memberType){
			m.isLoadingData = true;
			angular.forEach(m.playersArrays, function(player){
				playerBaseModelFactory.getPlayerInfo(memberType, player.displayName).then(function(response){
					m.fireTeamMembers[player] = response;
				});
			});
			m.isLoadingData = false;
			m.initialSearchRun = true;
		...
		function keyDownEvt(e){
			switch(e.keyCode){
				case 13:
					e.preventDefault();
					getFireTeamModel(m.selectedPlatform);
				break;
			}
		}
	};